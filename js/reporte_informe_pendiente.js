...
function iniciar(){ 
  ...
  $('.btn_paginacion').click( function (){   
    ...
    window.location.href = 'reporte_informe_pendiente.php?'+"pagina_actual="+pagina_actual+texto+"&fecha_ini="+$("#fecha_inicio").val()+"&fecha_fin="+$("#fecha_fin").val();    
  }); 
  ...
  $("#btn_buscar").click( function () {
    if( validar_datos() ){ 
      window.location.href = 'reporte_informe_pendiente.php?'+'texto='+$("#buscar_texto").val()+"&fecha_ini="+$("#fecha_inicio").val()+"&fecha_fin="+$("#fecha_fin").val();    
    } 
  });
  ...
  $("#btn_generar_excel").click( function (){
    ...
    window.open('services/reportes/informes_pendientes.php?data='+encodeURI(JSON.stringify(data)), '_blank');         
  });
}
